#!/usr/bin/env Rscript
#PBS -N 2b-duroc_ld_calculation
#PBS -l nodes=1:ppn=1,walltime=00:30:00,mem=10Gb
#PBS -j oe
#PBS -o ../output/2b-duroc_ld_calculation
#PBS -m abe
#PBS -t 1-20
#PBS -M funkhou9@msu.edu

#' # Script: 2b-duroc_ld_calculation
#'
...
geno <- read.table("../Duroc_fimpute_run/genotypes_imp.txt",
                   header = TRUE,
                   stringsAsFactors = FALSE,
                   colClasses = rep("character", 3))
geno <- geno[, c(1, 3)]

snps <- read.table("../Duroc_fimpute_run/snp_info.txt",
                   header = TRUE,
                   stringsAsFactors = FALSE)
...
write.table(snps_subset,
            file = paste0("../Duroc_fimpute_run/snp_info_",
                          job_num,
                          ".txt"),
            sep = '\t',
            row.names = FALSE,
...
write.table(geno_subset,
            file = paste0("../Duroc_fimpute_run/genotypes_imp_",
                          job_num,
                          ".txt"),
            sep = '\t',
            row.names = FALSE,
...
sink(paste0("../ld_Duroc_", job_num, ".ctr"))
cat(
  paste0(
'title
  "LD Duroc";
...
gfile
  "../Duroc_fimpute_run/genotypes_imp_', job_num, '.txt"
  skip 1;

mapfile
  "../Duroc_fimpute_run/snp_info_', job_num, '.txt"
  skip 1;
...
output_folder
  "../Duroc_snp1101_', out_num, '_run";\n'
  )
)
sink()

#' ### Submit executable
system(paste0("echo ../ld_Duroc_", job_num, ".ctr | ../src/snp1101_Scott170203/snp1101"))

#' Remove control file, which is no longer needed since it is stored automatically
#' in the output file
system(paste0("rm ../ld_Duroc_", job_num, ".ctr"))
