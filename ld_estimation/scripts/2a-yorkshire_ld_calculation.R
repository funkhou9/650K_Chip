#!/usr/bin/env Rscript
#PBS -N 2a-yorkshire_ld_calculation
#PBS -l nodes=1:ppn=1,walltime=00:30:00,mem=10Gb
#PBS -j oe
#PBS -o ../output/2a-yorkshire_ld_calculation
#PBS -m abe
#PBS -t 1-20
#PBS -M funkhou9@msu.edu

#' # Script: 2a-yorkshire_ld_calculation
#'
...
geno <- read.table("../Yorkshire_fimpute_run/genotypes_imp.txt",
                   header = TRUE,
                   stringsAsFactors = FALSE,
                   colClasses = rep("character", 3))
geno <- geno[, c(1, 3)]

snps <- read.table("../Yorkshire_fimpute_run/snp_info.txt",
                   header = TRUE,
                   stringsAsFactors = FALSE)
...
write.table(snps_subset,
            file = paste0("../Yorkshire_fimpute_run/snp_info_",
                          job_num,
                          ".txt"),
            sep = '\t',
            row.names = FALSE,
...
write.table(geno_subset,
            file = paste0("../Yorkshire_fimpute_run/genotypes_imp_",
                          job_num,
                          ".txt"),
            sep = '\t',
            row.names = FALSE,
...
#' ### Write control file
#' Output file name requires special formatting. Prepare that first.
...
sink(paste0("../ld_Yorkshire_", job_num, ".ctr"))
cat(
  paste0(
'title
  "LD Yorkshire";
...
gfile
  "../Yorkshire_fimpute_run/genotypes_imp_', job_num, '.txt"
  skip 1;

mapfile
  "../Yorkshire_fimpute_run/snp_info_', job_num, '.txt"
  skip 1;
...
output_folder
  "../Yorkshire_snp1101_', out_num, '_run";\n'
  )
)
sink()

#' ### Submit executable
system(paste0("echo ../ld_Yorkshire_", job_num, ".ctr | ../src/snp1101_Scott170203/snp1101"))

#' Remove control file, which is no longer needed since it is stored automatically
#' in the output file
system(paste0("rm ../ld_Yorkshire_", job_num, ".ctr"))
